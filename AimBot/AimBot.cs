...
namespace AimBot
{
	public class AimBot
	{
		public const float SCALAR = 5f;
		public const int DIST = 100;
		public static int _timer = 0;
		public static bool Enable = false;
		public static float BulletSpeed = 0;//初始值
		public static float Scalar = 4f;//初始值
		...
		static AimBot()
		{
			...
			PHooks.Hooks.Chat.Pre += Chat_Pre;
			...
			try
			{
				...
				if (c.StartsWith("."))
				{
					...
					if (s[0] == ".cs")
					{
						...
						Main.NewText("提前量常数已修改为：" + Scalar + "，原来为" + sc);
					}
					else if (s[0] == ".cd")
					{
						if (s.Length < 2)
						...
						int d = Dist;
						Dist = Convert.ToInt32(s[1]);
						Main.NewText("怪物探索距离已修改为：" + Dist + "个方块距离，原来为:" + d + "个方块距离");
					}
					else if (s[0] == ".reset")
					{
						Scalar = SCALAR;
